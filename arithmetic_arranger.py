...
def arithmetic_arranger(problems, displayMode=False):
    ...
    for prob in problems:
        separatedProblem = prob.split()
        number1 = separatedProblem[0]
        operator = separatedProblem[1]
        number2 = separatedProblem[2]
        ...
        if start == True:
            ...
            line3 += sideSpace + '-' * (space + 2)
            if displayMode == True:
                if operator == '+':
                    line4 += sideSpace + str(no1 + no2).rjust(space + 2)
                else:
                    line4 += sideSpace + str(no1 - no2).rjust(space + 2)
    if displayMode == True:
        return line1 + '\n' + line2 + '\n' + line3 + '\n' + line4
